...
namespace Swagger.Net
{
    public static class SwaggerGen
    {
        ...
        /// </summary>
        /// <param name="pi"></param>
        /// <param name="belongToType">当前参数所在的类的任意类型，用来寻找对应的xml doc</param>
        /// <returns></returns>
        public static string GetParameterDocumentation(ParameterInfo pi, Type belongToType = null)
        {
            if (belongToType == null)
                belongToType = pi.Member.DeclaringType;
            ...
            var parms = (pi.Member as MethodInfo).GetParameters();

            node = navi.SelectSingleNode(string.Format("/doc/members/member[@name='M:{0}.{1}({2})']/param[@name='{3}']",
               belongToType.FullName, pi.Member.Name, parms.Length > 0 ? string.Join(",", parms.Select(x => x.ParameterType.FullName)) : string.Empty, pi.Name));

            //node = navi.SelectSingleNode(string.Format("/doc/members/member[@name='M:{0}']/param[@name='{1}']", (pi.Member as System.Reflection.MethodInfo).FullName, pi.Name));
